...
from dbi3_access.lib.dbi3_common import utc, DBI_ALL_LOG_FIELDS, DBI_DEFAULT_LOG_FIELDS, DBI_CONF_FILE, DEF_LOG_PATH, \
    DEF_KML_PATH
...
def convert_new_logs(app_config):
    ...
    for le in conv_list.conversion_list:
        if le.new_file:  # only process files marked as new
            ...
            if rtn < 0:
                get_log().info('Convert FAILED {} to KML  new:{}  edits:{}   to {}\n{}'.format(le.log_name,
                ...
                get_log().info('Convert {} to KML  new:{}  edits:{}\n{}'.format(le.log_name,
                                                                       'Y' if le.new_file else 'N',
                                                                       le.override,
                                                                       rtn_str))
            elif rtn == 0:
                get_log().info('Converted {} to KML  new:{}  edits:{}\n{}'.format(le.log_name,
                                                                         'Y' if le.new_file else 'N',
                                                                         le.override,
                                                                         rtn_str))
...
def filter_text():
    ...
    fl = []
    if app_config.CLI_new_logs: fl.append('new_logs')
    if app_config.CLI_skip_invalid: fl.append('skip_invalid')
    if app_config.CLI_age_limit: fl.append('after-{}'.format(app_config.CLI_age_limit.strftime('%Y_%m_%d_%H_%M_%S')))
    return ':'.join(fl)
...
class Dbi3InteractiveCommandLine(cmd.Cmd):
    ...
    def do_filter(self, line):
        ...
        if line == '':
            print('new_logs:{}  age_limit:{} Days  valid_only:{}'.format(app_config.CLI_new_logs,
                                                                         app_config.age_limit,
                                                                         app_config.CLI_skip_invalid))
            return
            ...
            if ln == 'all':
                ...
                try:
                    ...
                    if age <= 0:
                        ...
                        app_config.CLI_age_limit = (datetime.utcnow() - timedelta(days=age)).replace(tzinfo=utc)
    ...
    def do_convert(self, line):
        ...
        for le in self.my_list:
            if le[0]:  # list row is marked as selected
                ...
                if rtn < 0:
                    get_log().info('Convert {} to {} FAILED: {}'.format(le[1].log_name, kml_name, rtn_str))
                elif rtn > 0 and app_config.verbose:
    ...
    def do_convert(self, line):
        ...
        for le in self.my_list:
            if le[0]:
                ...
                if rtn < 0:
                    get_log().info('Convert {} to KML {} FAILED: {}'.format(le[1].log_name, le[1].kml_name, rtn_str))
                elif rtn > 0 and app_config.verbose:
                    get_log().info('Convert {} to KML: {}'.format(le[1].log_name, rtn_str))
                elif rtn == 0:
                    get_log().info('Convert {} to KML\n{}'.format(le[1].log_name, rtn_str))
                ...
                if do_csv:
                    ...
                    if rtn < 0:
                        get_log().info('Convert {} to CSV {} FAILED: {}'.format(le[1].log_name, csv_filename, rtn_str))
                    elif rtn > 0 and app_config.verbose:
...
"""
    ...
    app_config = Dbi3ConfigOptions(altitudemode=args.altitudemode,
    ...
    if app_config.age_limit is not None:  # If age_limit is set, create the datetime equivelent
        app_config.CLI_age_limit = (datetime.utcnow() - timedelta(days=app_config.age_limit)).replace(tzinfo=utc)
    else:
        app_config.CLI_age_limit = None
