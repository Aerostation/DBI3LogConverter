...
"""namedtuple for the config file editor
        ...
        help_txt = help
"""
ConfigSpec = collections.namedtuple(
    "ConfigSpec", "field_name name default validation_func direct help_txt"
)
...
class Dbi3ConfigOptions:
    ...
    def __init__(
        ...
        # age_limit in days.  When age filter enabled, limit log/kml list outputs to newer
        self.age_limit = None
        ...
        if prefer_gps is not None:
            self.prefer_gps = prefer_gps
        if extend_to_ground is not None:
            self.extend_to_ground = extend_to_ground
        if fields_choice is not None:
            self.kml_fields = fields_choice
        if kml_use_metric is not None:
            self.kml_use_metric = kml_use_metric
    ...
    """
        ...
        AppConfig = [
            ...
            ConfigSpec(
                ...
                self.ask_for_port,
                True,
                "Serial port connected to the DBI3",
            ),
            ConfigSpec(
                "age_limit",
                "filter_age_limit",
            ...
            ConfigSpec(
                "kml_fields",
                "kml_fields",
                ",".join(DBI_DEFAULT_LOG_FIELDS),
                self.fields_check,
                False,
                "Default Google Earth extra data fields, available (ALL,{})".format(
                    ",".join(DBI_ALL_LOG_FIELDS)
                ),
            ),
            ConfigSpec(
                "verbose",
                ...
                self.true_false,
                False,
                "Enable verbose debug output? T/F",
            ),
        ]
    ...
    def edit_conversion_config(self, cfg_path):
        ...
        KmlConfig = [
            ...
            ConfigSpec(
                ...
                self.fields_check,
                False,
                "Google Earth extra data fields, available (ALL,{})".format(
                    ",".join(DBI_ALL_LOG_FIELDS)
                ),
            ),
        ]
    ...
    def _update_config_file(self, cfg_path, config_table, data):
        while True:
            ...
            for cfg_index, el in enumerate(config_table, 1):
                ...
                current_val = (
                    k if k is not None else "APP->{}".format(l) if l is not None else None
                )
                print(
                    "{:2} {:<24} {:<12}:  {}".format(
    ...
    def fields_check(param_name, fields):
        ...
        for fn in fl:
            if fn not in DBI_ALL_LOG_FIELDS:
                print(
                    'error: argument --fields: invalid choice: "{}" is not in "{}"'.format(
                        fn, DBI_ALL_LOG_FIELDS
                    )
                )
                return None
        return fl
...
class Dbi3ConversionOptions:
    ...
    def __init__(self, filename, app_config, altitude_offset=None):
        ...
        self.log_meta = os.path.join(
            os.path.dirname(filename), "." + os.path.splitext(os.path.basename(filename))[0]
        )
    ...
    def __str__(self):
        return (
            "log_filename:{} altitudemode:{} altitude_offset:{} extend_to_ground:{} kml_fields:{}"
